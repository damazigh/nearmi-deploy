...
function prepare_env() {
  ...
  log "success" "preparation complete"
}
...
function install_k8s() {
  ...
  apt update;
  install_pckg "kubelet" "${K8S_VERSION}"
  install_pckg "kubeadm" "${K8S_VERSION}"
  if [ $1 = "master" ];
  then
    install_pckg "kubectl" "${K8S_VERSION}"
  fi
}
...
function init_master_node() {
  ...
  if id "${K8S_MASTER_USER}" &>/dev/null; then
    log "warn" "user exists (assuming group : ${K8S_MASTER_GROUP} exists too)"
  else
